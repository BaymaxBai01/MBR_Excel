...
def get_page_views_1(client, area):
    kusto_query_1 = "database('DevRelWorkArea').['kpi-visitor'] " \
                    "| where Area == '" + area + "' " \
...
def get_page_views_5(client, area):
    kusto_query_5 = "database('DevRelWorkArea').['kpi-viewed-article'] " \
                    "| where Locale == 'en-us' "\
                    "| where Area == '" + area + "' " \
                    "| where Month >= startofmonth(now(),-14) and Month < startofmonth(now()) " \
                    "| summarize val = avg(MTD) by Month " \
...
def get_page_views_6(client, area):
    kusto_query_6 = "database('DevRelWorkArea').['kpi-visitor-by-visitor-type'] " \
                    "| where Area == '" + area + "' " \
                    "| where Month >= startofmonth(now(),-14) and Month < startofmonth(now()) " \
                    "| summarize val = avg(mtdengaged) by Month " \
                    "| order by Month desc "
...
def get_page_views_9(client, area):
    kusto_query_9 = "database('DevRelWorkArea').['kpi-deploy-to-azure'] " \
                    "| where Area == '" + area + "' " \
                    "| where Month >= startofmonth(now(),-14) and Month < startofmonth(now()) " \
...
def get_page_views_10(client, area):
    kusto_query_10 = "database('DevRelWorkArea').['kpi-github-click'] " \
                     "| where Area == '" + area + "' " \
                     "| where Month >= startofmonth(now(),-14) and Month < startofmonth(now()) " \
                     "| summarize val = avg(uv_clicked) by Month " \
                     "| order by Month desc "
...
def get_page_views_11(client, area):
    kusto_query_11 = "database('DevRelWorkArea').['kpi-total-article'] " \
                     "| where Locale == 'en-us' "\
                     "| where Area == '" + area + "' " \
                     "| where Month >= startofmonth(now(),-14) and Month < startofmonth(now()) " \
                     "| summarize val = avg(articles) by Month " \
                     "| order by Month desc "
...
def get_page_views_12(client, area):
    kusto_query_12 = "database('DevRelWorkArea').['kpi-new-article'] " \
                     "| where Locale == 'en-us' "\
                     "| where Area == '" + area + "' " \
                     "| where Month >= startofmonth(now(),-14) and Month < startofmonth(now()) " \
                     "| summarize val = avg(articles) by Month " \
...
def get_page_views_13(client, area):
    kusto_query_13 = "database('DevRelWorkArea').['kpi-updated-article'] " \
                     "| where Locale == 'en-us' "\
                     "| where Area == '" + area + "' " \
                     "| where Month >= startofmonth(now(),-14) and Month < startofmonth(now()) " \
                     "| summarize val = avg(MTD) by Month " \
...
def get_page_views_14(client, area):
    kusto_query_14 = "database('DevRelWorkArea').['kpi-visitor-voted'] " \
                     "| where Area == '" + area + "' " \
                     "| where Month >= startofmonth(now(),-14) and Month < startofmonth(now()) " \
                     "| summarize val = avg(mtdCSAT)/100 by Month " \
                     "| order by Month desc "
...
ls=[cga_client, "Azure Architecture Center"]
...
y = 23
for i in range(14):
    sheet_x.cell(x, y, current_1.iloc[i][j]) #32行表头
    sheet_x.cell(x + 1, y, current_1.iloc[i][j + 1]) #33行
    sheet_x.cell(x + 2, y, current_2.iloc[i][j + 1]) #34行
    sheet_x.cell(x + 3, y, current_3.iloc[i][j + 1]) #35行
    sheet_x.cell(x + 4, y, current_4.iloc[i][j + 1]) #36行
    sheet_x.cell(x + 5, y, current_5.iloc[i][j + 1]) #37行
    sheet_x.cell(x + 7, y, current_6.iloc[i][j + 1]) #39
    sheet_x.cell(x + 8, y, current_7.iloc[i][j + 1]) #40
    sheet_x.cell(x + 9, y, current_8.iloc[i][j + 1]) #41
    sheet_x.cell(x + 10, y, current_9.iloc[i][j + 1]) #42
    sheet_x.cell(x + 11, y, current_10.iloc[i][j + 1]) #43
    sheet_x.cell(x + 13, y, current_11.iloc[i][j + 1]) #45
    sheet_x.cell(x + 14, y, current_12.iloc[i][j + 1]) #46
    sheet_x.cell(x + 15, y, current_13.iloc[i][j + 1]) #47
    sheet_x.cell(x + 16, y, current_14.iloc[i][j + 1]) #48
    y += -1
    i += 1
