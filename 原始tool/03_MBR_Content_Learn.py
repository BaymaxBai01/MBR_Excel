...
def get_page_views_9(client, area):
    kusto_query_9 = "database('DevRelWorkArea').['kpi-new-article'] " \
                    "| where Locale == 'en-us' "\
                    "| where Area == '" + area + "' " \
                    "| where Month >= startofmonth(now(),-14) and Month < startofmonth(now()) " \
...
y = 23
for i in range(14):
    sheet_x.cell(x, y, current_1.iloc[i][j])
    sheet_x.cell(x + 1, y, current_1.iloc[i][j + 1])
    sheet_x.cell(x + 2, y, current_2.iloc[i][j + 1])
    sheet_x.cell(x + 3, y, current_3.iloc[i][j + 1])
    sheet_x.cell(x + 4, y, current_4.iloc[i][j + 1])
    sheet_x.cell(x + 6, y, current_5.iloc[i][j + 1])
    sheet_x.cell(x + 7, y, current_6.iloc[i][j + 1])
    sheet_x.cell(x + 8, y, current_7.iloc[i][j + 1])
    sheet_x.cell(x + 9, y, current_8.iloc[i][j + 1])
    sheet_x.cell(x + 11, y, current_9.iloc[i][j + 1])
    sheet_x.cell(x + 12, y, current_10.iloc[i][j + 1])
    sheet_x.cell(x + 13, y, current_11.iloc[i][j + 1])
    sheet_x.cell(x + 15, y, current_12.iloc[i][j + 1])
    sheet_x.cell(x + 16, y, current_13.iloc[i][j + 1])
    sheet_x.cell(x + 17, y, current_14.iloc[i][j + 1])
    y += -1
    i += 1
